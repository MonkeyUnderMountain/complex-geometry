...
We can define its \emph{sheaf of differential forms}.

For regular locus, this is easy and standard.

\begin{definition}
	Let $A \subset B$ be a local model.
	Its \emph{sheaf of differential forms} is defined as
	\[ \Omega_A \coloneqq (\Omega_B)|_A \Big/ (\scrI_A/\scrI_A^2). \]
	It is a coherent $\scrO_A$-module.
	\Yang{}
\end{definition}

\Yang{Here recall the first/second exact sequence of differential}

The elements in $\Omega_A$ can be viewed as the differential forms on regular locus that can extend to global $B$.

\Yang{up to now, i don't know whether this sheaf is intrinct.}


\Yang{need to rewrite the following in the settings of analytic varieties}
...
\begin{proposition}\label{prop:A^k_is_Frechet_space}
