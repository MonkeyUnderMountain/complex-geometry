...
In this section, all vector bundles and line bundles are geometric vector bundles unless emphasis.
Recall that for a locally free sheaf $\calE$, its geometric vector bundle is given by $E = \bbV(\calE)$.
And if you take the sheaf of sections of $E$, then you get the dual sheaf $\calE^\vee$ of original sheaf $\calE$.

We will try to work on the settings of complex analytic space.

\subsection{The first properties}

Let \(X\) be a complex analytic space and \(E \to X\) a (geometric) smooth complex vector bundle.

\begin{definition}\label{def:hermitian-metric-on-holomorphic-vector-bundles}
	A \emph{smooth hermitian metric} (resp. \emph{continous hermitian metric}) on \(E\) is a smoothly (resp. continuously) varying family of hermitian inner products \(\langle \cdot, \cdot \rangle_x\) on the fibers \(E_x\) for each \(x \in X\),
	i.e.,
	...
	A smooth complex vector bundle equipped with a smooth hermitian metric is called a \emph{hermitian vector bundle}.
\end{definition}

\begin{definition}\label{def:hermitian-metric-on-tangent-bundle}
...
\begin{proposition}\label{prop:hermitian_metric_different_by_a_smooth_function_on_a_fixed_line_bundle}
	...
	Then there exists a unique smooth function \(\varphi: X \to \bbR\) such that
...
\begin{definition}\label{def:green-functions-for-a-divisor}
	...
	A \emph{green function} of $D$,

	\Yang{to be continued}
\end{definition}

\begin{proposition}
	There is an equation of currents:
	\[ \upd \upd^c g = \omega_h - \delta_D. \]
	\Yang{to be continued.}
\end{proposition}
...
\Yang{now we extend these theory to vector bundle}
